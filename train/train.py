...
from models.deep_aai_kmer_pssm_embedding_cls import DeepAAIKmerPssmEmbeddingCls
import numpy as np
import random
import torch
import math
...
class Trainer(object):
    ...
    def iteration(self, epoch, pair_idx, antibody_graph_node_idx, virus_graph_node_idx, is_training=True, shuffle=True):
        ...
        for i in range(math.ceil(pair_num/self.param_dict['batch_size'])):
            ...
            ft_dict = {
                'antibody_graph_node_kmer_ft': antibody_graph_node_kmer_ft,     # abs dataset kmer
                'virus_graph_node_kmer_ft': virus_graph_node_kmer_ft,           
                'antibody_graph_node_pssm_ft': antibody_graph_node_pssm_ft,     # abs dataset pssm
                'virus_graph_node_pssm_ft': virus_graph_node_pssm_ft,
                'antibody_amino_ft': batch_antibody_amino_ft,                   # one-hot
            ...
            all_label = np.hstack([all_label, batch_label]).astype(np.int64)
        # print(all_loss)
...
if __name__ == '__main__':
    for seed in range(20):
        ...
        param_dict = {
            ...
            'add_res': True,
            'add_bn': False,
            'max_len': 344,    # antibody:344, virus:912
        }
